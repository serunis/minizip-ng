...
int32_t mz_os_replace_resolved(const char *source_path, const char *replace_path) {
    ...
    uint8_t source_is_target = 0;

    if (!source_path || !replace_path)
        return MZ_PARAM_ERROR;

    err = mz_os_path_same_file(source_path, replace_path);
    if (err == MZ_OK)
        return MZ_OK;
    if (err == MZ_MEM_ERROR)
        return err;
    ...
    if (target_result == 0 && !S_ISLNK(target_link_stat.st_mode)) {
        ...
        if (err == MZ_OK && fstat(source_fd, &source_stat) != 0)
            err = MZ_READ_ERROR;
        if (err == MZ_OK && source_stat.st_dev == target_stat.st_dev && source_stat.st_ino == target_stat.st_ino)
            source_is_target = 1;
        else if (err == MZ_OK && target_stat.st_nlink > 1) {
#if defined(O_NOFOLLOW)
