...
class WriterPathTest : public testing::Test {
  protected:
    void SetUp() override {
        ...
        non_split_path = test_dir + "/z-output.z1";
    ...
    void TearDown() override {
        ...
        std::remove(non_split_path.c_str());
...
TEST_F(WriterPathTest, split_output_does_not_exclude_non_segment_numeric_extension) {
    void *reader = nullptr;
    void *writer = nullptr;
    mz_zip_file *file_info = nullptr;
    int32_t err = MZ_OK;
    ...
    {
        std::ofstream input(non_split_path, std::ios::binary);
        ASSERT_TRUE(input.is_open());
        input << "not a split archive segment";
    }
    ...
    while (err == MZ_OK) {
        ASSERT_EQ(MZ_OK, mz_zip_reader_entry_get_info(reader, &file_info));
        EXPECT_STREQ("z-output.z1", file_info->filename);
        entry_count += 1;
        err = mz_zip_reader_goto_next_entry(reader);
    }
    EXPECT_EQ(MZ_END_OF_LIST, err);
...
TEST_F(WriterPathTest, excludes_existing_hard_link_to_split_output) {
    void *writer = nullptr;
    std::string split_path = test_dir + "/z-output.z01";

    CreateInputFiles();
    {
        std::ofstream split(split_path, std::ios::binary);
        ASSERT_TRUE(split.is_open());
        split << "existing split archive";
    }
#if defined(_WIN32)
    if (!CreateHardLinkA(alias_path.c_str(), split_path.c_str(), NULL))
        GTEST_SKIP() << "Creating hard links is not supported by the test filesystem";
#else
    ASSERT_EQ(0, link(split_path.c_str(), alias_path.c_str()));
#endif
...
TEST_F(WriterPathTest, replacing_symlink_to_same_file_is_noop) {
    std::ifstream target;
    std::string contents;
    ...
    ASSERT_EQ(MZ_OK, mz_os_replace(alias_path.c_str(), excluded_path.c_str()));
...
TEST_F(WriterPathTest, replacing_hard_link_with_same_file_is_noop_on_windows) {
    std::ifstream target;
    std::ifstream alias;
    std::string target_contents;
    std::string alias_contents;
    ...
    ASSERT_EQ(MZ_OK, mz_os_replace(alias_path.c_str(), excluded_path.c_str()));
    EXPECT_EQ(MZ_OK, mz_os_file_exists(alias_path.c_str()));
    ...
    target_contents.assign(std::istreambuf_iterator<char>(target), std::istreambuf_iterator<char>());
    alias_contents.assign(std::istreambuf_iterator<char>(alias), std::istreambuf_iterator<char>());
    EXPECT_EQ("existing archive", target_contents);
    EXPECT_EQ(target_contents, alias_contents);
}
#endif
...
TEST_F(WriterPathTest, replacing_hard_link_with_same_file_is_noop) {
    struct stat target_stat;
    struct stat alias_stat;
    std::ifstream target;
    std::ifstream alias;
