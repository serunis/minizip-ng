...
typedef struct mz_zip_writer_s {
    ...
    mz_zip_writer_path *split_path_head;
    mz_zip_writer_path *split_path_tail;
    const char *password;
    const char *comment;
...
static int32_t mz_zip_writer_capture_split_paths(void *handle, const char *archive_path);
...
static int32_t mz_zip_writer_open_file_int(void *handle, const char *path, int64_t disk_size, uint8_t append,
    ...
    if (disk_size > 0) {
        err = mz_zip_writer_capture_split_paths(writer, path);
        if (err != MZ_OK) {
            mz_zip_writer_close(writer);
            return err;
        }
...
int32_t mz_zip_writer_close(void *handle) {
    ...
    mz_zip_writer_path_list_delete(&writer->split_path_head, &writer->split_path_tail);
    writer->disk_size = 0;
    return err;
}
...
static int32_t mz_zip_writer_split_path_matches(const char *path, const char *archive_path) {
    ...
    path_ext_ptr = path_ext + 2;
    disk_number_length = strlen(path_ext_ptr);
    if (disk_number_length < 2 ||
        (path_ext_ptr[0] == '0' && (disk_number_length != 2 || path_ext_ptr[1] == '0')))
        return MZ_EXIST_ERROR;

    for (; *path_ext_ptr != 0; path_ext_ptr += 1) {
...
static int32_t mz_zip_writer_path_matches(void *handle, const char *path, const char *archive_path) {
    mz_zip_writer *writer = (mz_zip_writer *)handle;
    mz_zip_writer_path *split_path = NULL;
    int32_t err = MZ_EXIST_ERROR;
    ...
    split_path = writer->split_path_head;
    while (split_path) {
        err = mz_os_path_same_file(path, split_path->path);
        if (err == MZ_OK || err == MZ_MEM_ERROR)
            return err;
    ...
    if (writer->disk_size == 0)
        return MZ_EXIST_ERROR;
    return mz_zip_writer_split_path_matches(path, archive_path);
}
...
static int32_t mz_zip_writer_capture_split_paths(void *handle, const char *archive_path) {
    ...
    if (!writer || !archive_path)
        return MZ_PARAM_ERROR;

    directory = (char *)strdup(archive_path);
    if (!directory)
        return MZ_MEM_ERROR;
    mz_path_remove_filename(directory);
    if (directory[0] == 0) {
    ...
    while ((entry = mz_os_read_dir(dir))) {
        ...
        err = mz_zip_writer_path_combine_alloc(directory, entry->d_name, &full_path);
        if (err != MZ_OK)
            break;
        err = mz_zip_writer_split_path_matches(full_path, archive_path);
        if (err == MZ_OK)
            err = mz_zip_writer_path_list_append(&writer->split_path_head, &writer->split_path_tail, full_path,
                                                 full_path);
        else if (err != MZ_MEM_ERROR)
            err = MZ_OK;
        free(full_path);
        full_path = NULL;
        if (err != MZ_OK)
